...
class WuaIndividualinput(models.Model):
    ...
    effective_volume_vol_hours = fields.Char(
        string='Effective Volume',
        compute='_compute_effetive_volume_vol_hours')
    ...
    @api.depends('effective_volume', 'effective_volume_hours')
    def _compute_effetive_volume_vol_hours(self):
        for record in self:
            effective_volume_vol_hours = self.env[
                'wua.quota'].transform_to_quota_hours_format_form_view(
                    record.effective_volume, record.effective_volume_hours)
            record.effective_volume_vol_hours = effective_volume_vol_hours
