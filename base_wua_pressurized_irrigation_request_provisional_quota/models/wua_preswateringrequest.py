...
class WuaPreswateringrequest(models.Model):
    ...
    @api.onchange('partner_id', 'agriculturalseason_id')
    def _onchange_partner_agricultural_id(self):
        ...
        if current_quotaperiod:
            ...
            if quotas:
                ...
                for quota in quotas:
                    superproduct = '<b>' + quota.superproduct_id.name + '</b>'
                    balance = self.env['wua.parcel'].transform_float_to_locale(
                        quota.provisional_balance, 2)
                    balance = balance + _(' m³')
                    color_to_use = 'unset'
                    if quota.provisional_balance < 0:
                        color_to_use = 'red'
                    elif quota.provisional_balance > 0:
                        color_to_use = 'blue'
                    balance = '<span style="color:' + color_to_use + '">' + \
                        balance + '</span>'
                    body = body + '<br/>' + superproduct + ' : ' + balance
