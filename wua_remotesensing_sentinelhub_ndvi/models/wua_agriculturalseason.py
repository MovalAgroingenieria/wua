...
class WuaAgriculturalseason(models.Model):
    ...
    def write(self, vals):
        if 'active_agriculturalseason' in vals:
            self._update_active_flag_in_ndvi_model(
    ...
    # "wua.parcel.vegetationindex.ndvi" model.
    def _update_active_flag_in_ndvi_model(self, agriculturalseason_id,
                                          active_agriculturalseason):
        if agriculturalseason_id:
            try:
                self.env.cr.savepoint()
                self.env.cr.execute("""
                    UPDATE wua_parcel_vegetationindex_ndvi
    ...
    def do_global_assignment(self):
        ...
        measurements = self.env['wua.parcel.vegetationindex.ndvi'].search([])
