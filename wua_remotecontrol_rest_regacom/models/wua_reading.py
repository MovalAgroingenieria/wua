...
class WuaReading(models.Model):
    ...
    @api.model
    def run_remotecontrol_application_url_regacom(self):
    ...
    # Implemented hook
    def populate_data_for_import_readings_regacom(
    ...
    # Implemented hook
    def import_readings_regacom(
        self, url_remotecontrol_rest, url_remotecontrol_rest_username,
            url_remotecontrol_rest_password, list_of_data):
        readings = []
        error_message = ''
        error_watermeters = []
        # Get last readings from Regacom
        last_readings = self._get_regacom_last_readings()
        if not last_readings:
            error_message = _('No readings found in Regacom database')
            return readings, error_message, error_watermeters
        # Get configured watermeters
        watermeters = self._get_regacom_watermeters()
        # Process each watermeter
        for wm in watermeters:
            result = self._process_watermeter_reading(
                wm, last_readings)
    ...
    def _get_regacom_last_readings(self):
        try:
            procedure = self.env.ref(
                'remotecontrol_regacom.'
                'remotecontrol_regacom_procedure_last_readings')
        except Exception:
            return {}
        try:
            bag = {}
    ...
    def _get_regacom_watermeters(self):
        return self.env['wua.watermeter'].search([
            ...
            ('waterconnection_id.telecontrol_associated', '=', 'regacom'),
            ('waterconnection_id.regacom_position', '!=', False),
        ])

    def _process_watermeter_reading(self, wm, last_readings):
        result = {'reading': None, 'error': None}
        try:
            dir_um = wm.irrigationshed_id.regacom_dirum
            dir_iru = wm.irrigationshed_id.regacom_diriru
            position = wm.waterconnection_id.regacom_position
            # Check if reading exists
            if not self._reading_exists(
                    last_readings, dir_um, dir_iru, position):
                result['error'] = _(
                    'No reading found for UM:%s IRU:%s POS:%s',
                ) % (dir_um, dir_iru, position)
                return result
            reading_data = last_readings[dir_um][dir_iru][position]
            volume = reading_data.get('value', 0.0)
            reading_date = reading_data.get('date')
            if not reading_date:
                return result
            # Check if reading is newer
            if self._is_reading_newer(reading_date, wm.last_reading_time):
                result['reading'] = {
                    'watermeter': wm.name,
                    'volume': volume,
                }
    ...
    def _reading_exists(self, last_readings, dir_um, dir_iru, position):
        return (dir_um in last_readings and
                dir_iru in last_readings[dir_um] and
                position in last_readings[dir_um][dir_iru])

    def _is_reading_newer(self, reading_date, last_reading_time):
        try:
    ...
    def do_import_reading_of_telecontrol(self):
        ...
        import_from_readings = self.env['ir.values'].get_default(
            'wua.irrigation.configuration', 'import_from_readings_regacom')
        if (import_from_readings and url_remotecontrol_rest):
            try:
                data = self.populate_data_for_import_readings_regacom(
                    url_remotecontrol_rest,
                    url_remotecontrol_rest_username,
                    url_remotecontrol_rest_password)
                if data:
                    readings, error_message, error_watermeters = \
                        self.import_readings_regacom(
                            url_remotecontrol_rest,
                            url_remotecontrol_rest_username,
