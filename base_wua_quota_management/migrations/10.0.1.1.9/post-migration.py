        ...
        cr.execute("""
            CREATE MATERIALIZED VIEW wua_quota_aggregatevalue AS (
                SELECT
                    ...
                    SUM(q.accumulated_consumption)
                        AS accumulated_consumption,
                    SUM(q.accumulated_input)
                        - SUM(q.accumulated_consumption) AS balance
                FROM wua_quota q
                INNER JOIN res_partner p ON q.partner_id = p.id
                LEFT JOIN (
                    SELECT
                        partner_id,
                        SUM(area_official_water_costs_net) AS total_area
