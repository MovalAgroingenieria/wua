...
class WizardGeneratePeriods(models.TransientModel):
    _name = 'wizard.generate.periods'
    _description = 'Dialog box to generate watering periods'
            ...
            if (self.initial_date < agriculturalseason.initial_date or
               self.end_date > agriculturalseason.end_date):
                raise exceptions.UserError(_('The range of dates must be '
                                             ...
                                             'season.'))
            self.create_periods(agriculturalseason, self.initial_date,
    ...
    def create_periods(self, agriculturalseason,
        ...
        wateringperiods = self.env['wua.wateringperiod']
        while period_initial_date <= end_date and period_end_date <= end_date:
            wateringperiods.create({
