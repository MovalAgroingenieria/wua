...
class WizardPartnerFile(models.TransientModel):
    _name = 'wizard.partner.file'
    ...
    def _default_selected_parcels(self):
        ...
        for active_id in self.env.context['active_ids']:
            ...
            for parcel in partner.parcel_ids:
    ...
    def _default_selected_partners(self):
        ...
        for active_id in self.env.context['active_ids']:
            partner = self.env['res.partner'].browse(active_id)
            selected_partners_list.append(partner.display_name)
    ...
    def _compute_selected_parcels(self):
        for record in self:
            ...
            for active_id in record.env.context['active_ids']:
                partner = self.env['res.partner'].browse(active_id)
                for parcel in partner.parcel_ids:
                    parcel_ids_list.append(parcel.id)
    ...
    def _compute_selected_partners(self):
        for record in self:
            partner_ids_list = []
            for active_id in record.env.context['active_ids']:
                partner_ids_list.append(active_id)
